...
import {IBrunoLpLockerFeeConversion} from "./interfaces/IBrunoLpLockerFeeConversion.sol";
...
contract BrunoLpLockerFeeConversion is IBrunoLpLockerFeeConversion, ReentrancyGuard, Ownable {
    ...
    constructor(
        address owner_,
        address factory_, // Address of the bruno factory
        address feeLocker_,
        ...
        factory = factory_;
        feeLocker = IBrunoFeeLocker(feeLocker_);
    ...
    function placeLiquidity(
        IBruno.LockerConfig memory lockerConfig,
        IBruno.PoolConfig memory poolConfig,
        ...
        FeeIn[] memory lpFeeConversionPreferences = abi.decode(
            lockerConfig.lockerData, (IBrunoLpLockerFeeConversion.LpFeeConversionInfo)
        ).feePreference;
    ...
    function _mintLiquidity(
        ...
        for (uint256 i = 0; i < lockerConfig.tickLower.length; i++) {
            ...
            if (lockerConfig.tickLower[i] < poolConfig.tickIfToken0IsBruno) {
        ...
        bool token0IsBruno = token < poolConfig.pairedToken;
        ...
        int24 startingTick =
            token0IsBruno ? poolConfig.tickIfToken0IsBruno : -poolConfig.tickIfToken0IsBruno;
        ...
        for (uint256 i = 0; i < lockerConfig.tickLower.length; i++) {
            ...
            uint256 amount0 = token0IsBruno ? tokenAmount : 0;
            uint256 amount1 = token0IsBruno ? 0 : tokenAmount;

            // determine tick bounds for this position
            int24 tickLower_ =
                token0IsBruno ? lockerConfig.tickLower[i] : -lockerConfig.tickLower[i];
            int24 tickUpper_ =
                token0IsBruno ? lockerConfig.tickUpper[i] : -lockerConfig.tickUpper[i];
            int24 tickLower = token0IsBruno ? tickLower_ : tickUpper_;
            int24 tickUpper = token0IsBruno ? tickUpper_ : tickLower_;
    ...
    function _mevModuleOperating(address token) internal view returns (bool) {
        ...
        if (!IBrunoHook(address(_tokenRewards[token].poolKey.hooks)).mevModuleEnabled(poolId)) {
            return false;
        }
        ...
        uint256 poolCreationTimestamp =
            IBrunoHook(address(_tokenRewards[token].poolKey.hooks)).poolCreationTimestamp(poolId);
        if (
            poolCreationTimestamp
                + IBrunoHook(address(_tokenRewards[token].poolKey.hooks)).MAX_MEV_MODULE_DELAY()
                <= block.timestamp
        ) {
            return false;
        }
        ...
        returns (uint256[] memory, uint256[] memory)
    {
        ...
        // if the rewardToken is the bruno, we want to swap for recipients who
        // want their fee in the paired token
        //
        // conversely, if the rewardToken is the paired token, we want to swap for
        // recipients who want their fee in the bruno token
        FeeIn toSwap = token == rewardToken ? FeeIn.Paired : FeeIn.Bruno;
    ...
    function onERC721Received(address, address from, uint256 id, bytes calldata)
        ...
        returns (bytes4)
    {
        // Only Bruno Factory can send NFTs here
        if (from != factory) {
            revert Unauthorized();
        }
    ...
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return interfaceId == type(IERC721Receiver).interfaceId
            || interfaceId == type(IBrunoLpLocker).interfaceId;
    }
}