...
interface IBrunoPresaleEthToCreator is IBrunoExtension {
    event PresaleStarted(
        uint256 presaleId,
        address allowlist,
        IBruno.DeploymentConfig deploymentConfig,
        uint256 minEthGoal,
        uint256 maxEthGoal,
    ...
    event BrunoFeeRecipientUpdated(address oldRecipient, address recipient);
    event MinLockupDurationUpdated(uint256 oldMinLockupDuration, uint256 minLockupDuration);
    event BrunoDefaultFeeUpdated(uint256 oldBrunoDefaultFee, uint256 brunoDefaultFee);
    event BrunoFeeUpdatedForPresale(uint256 presaleId, uint256 oldBrunoFee, uint256 brunoFee);
    event SetAllowlist(address allowlist, bool enabled);

    struct Presale {
        PresaleStatus status; // current status of the presale
        IBruno.DeploymentConfig deploymentConfig; // token to be deployed upon successful presale
        ...
        uint256 brunoFee; // bruno's fee to take in weth on successful presale
    }
