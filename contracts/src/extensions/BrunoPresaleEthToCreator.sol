...
import {IBrunoExtension} from "../interfaces/IBrunoExtension.sol";
import {IBrunoPresaleAllowlist} from "./interfaces/IBrunoPresaleAllowlist.sol";
import {IBrunoPresaleEthToCreator} from "./interfaces/IBrunoPresaleEthToCreator.sol";
...
contract BrunoPresaleEthToCreator is ReentrancyGuard, IBrunoPresaleEthToCreator, OwnerAdmins {
    IBruno public immutable factory;
    ...
    // bruno fee info
    uint256 public brunoDefaultFeeBps;
    uint256 public constant BPS = 10_000;
    address public brunoFeeRecipient;
    ...
    constructor(address owner_, address factory_, address brunoFeeRecipient_)
        OwnerAdmins(owner_)
    {
        factory = IBruno(factory_);
        _presaleId = 1;
        brunoFeeRecipient = brunoFeeRecipient_;
        minLockupDuration = 7 days;
        brunoDefaultFeeBps = 500; // 5%
    }
    ...
    function setBrunoDefaultFee(uint256 brunoDefaultFeeBps_) external onlyOwner {
        if (brunoDefaultFeeBps_ >= BPS) revert InvalidBrunoFee();

        uint256 oldFee = brunoDefaultFeeBps;
        brunoDefaultFeeBps = brunoDefaultFeeBps_;

        emit BrunoDefaultFeeUpdated(oldFee, brunoDefaultFeeBps_);
    }

    function setBrunoFeeForPresale(uint256 presaleId, uint256 newFee)
        external
        presaleExists(presaleId)
        onlyOwner
    {
        // can only set lower
        if (newFee >= presaleState[presaleId].brunoFee) revert InvalidBrunoFee();

        uint256 oldFee = presaleState[presaleId].brunoFee;
        presaleState[presaleId].brunoFee = newFee;
        emit BrunoFeeUpdatedForPresale(presaleId, oldFee, newFee);
    }
    ...
    function setBrunoFeeRecipient(address recipient) external onlyOwner {
        address oldRecipient = brunoFeeRecipient;
        brunoFeeRecipient = recipient;
        emit BrunoFeeRecipientUpdated(oldRecipient, recipient);
    }

    function startPresale(
        IBruno.DeploymentConfig memory deploymentConfig,
        uint256 minEthGoal,
        uint256 maxEthGoal,
        ...
        if (allowlist != address(0)) {
            IBrunoPresaleAllowlist(allowlist).initialize(
        ...
        emit PresaleStarted({
            ...
            lockupDuration: lockupDuration,
            vestingDuration: vestingDuration,
            brunoFeeBps: brunoDefaultFeeBps
        });
    }
        ...
        nonReentrant
    {
        ...
        if (presale.allowlist != address(0)) {
            uint256 allowedAmount = IBrunoPresaleAllowlist(presale.allowlist)
    ...
    function claimEth(uint256 presaleId, address recipient) external presaleExists(presaleId) {
        ...
        // determine fee
        uint256 fee = (presale.ethRaised * presale.brunoFee) / BPS;
        ...
        // send eth to bruno
        if (fee > 0) {
            (bool sent,) = payable(brunoFeeRecipient).call{value: fee}("");
            if (!sent) revert EthTransferFailed();
        }
    ...
    function receiveTokens(
        IBruno.DeploymentConfig calldata deploymentConfig,
        PoolKey memory,
        address token,
        ...
        if (deploymentConfig.extensionConfigs[extensionIndex].msgValue != 0 || msg.value != 0) {
            revert IBrunoExtension.InvalidMsgValue();
        }
    ...
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return interfaceId == type(IBrunoExtension).interfaceId;
    }
}
