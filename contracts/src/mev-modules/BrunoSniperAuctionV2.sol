...
import {IBrunoFeeLocker} from "../interfaces/IBrunoFeeLocker.sol";

import {IBrunoHookV2} from "../hooks/interfaces/IBrunoHookV2.sol";
import {IBrunoLpLocker} from "../interfaces/IBrunoLpLocker.sol";
import {IBrunoMevModule} from "../interfaces/IBrunoMevModule.sol";

import {IBrunoMevDescendingFees} from "./interfaces/IBrunoMevDescendingFees.sol";
import {IBrunoSniperAuctionV0} from "./interfaces/IBrunoSniperAuctionV0.sol";
...
contract BrunoSniperAuctionV2 is
    ReentrancyGuard,
    IBrunoSniperAuctionV0,
    IBrunoMevDescendingFees,
    Ownable
{
    ...
    IBruno public immutable brunoFactory;
    IBrunoFeeLocker public immutable feeLocker;

    constructor(address owner_, address _brunoFactory, address _feeLocker, address _weth)
        Ownable(owner_)
    {
        brunoFactory = IBruno(_brunoFactory);
        feeLocker = IBrunoFeeLocker(_feeLocker);
        weth = _weth;
        ...
        view
    {
        ...
        // ensure that the associated hook is a BrunoHookV2
        if (
            !IBrunoHookV2(address(poolKey.hooks)).supportsInterface(
                type(IBrunoHookV2).interfaceId
            )
        ) {
            revert OnlyBrunoHookV2();
        }

        // ensure the starting fee is not greater than the max mev LP fee
        if (feeConfigData.startingFee > IBrunoHookV2(address(poolKey.hooks)).MAX_MEV_LP_FEE()) {
        ...
        if (
            feeConfigData.secondsToDecay
                > IBrunoHookV2(address(poolKey.hooks)).MAX_MEV_MODULE_DELAY()
        ) {
        ...
        onlyHook(poolKey)
    {
        ...
        IBrunoMevDescendingFees.FeeConfig memory feeConfigData =
            abi.decode(descendingFeeConfig, (IBrunoMevDescendingFees.FeeConfig));
    ...
    function _sendPayment(PoolKey calldata poolKey, bool brunoIsToken0, uint256 paymentAmount)
        internal
    {
        if (paymentAmount == 0) {
            return;
        ...
        SafeERC20.safeTransfer(IERC20(weth), address(brunoFactory), factoryPayment);

        address bruno = brunoIsToken0
            ? Currency.unwrap(poolKey.currency0)
            : Currency.unwrap(poolKey.currency1);

        // grab locker address from factory
        address lpLocker = brunoFactory.tokenDeploymentInfo(bruno).locker;

        // get reward info from the locker
        IBrunoLpLocker.TokenRewardInfo memory tokenRewardInfo =
            IBrunoLpLocker(lpLocker).tokenRewards(bruno);
    ...
    function _setLpFee(PoolKey calldata poolKey, uint24 lpFee) internal {
        ...
        IBrunoHookV2(msg.sender).mevModuleSetFee(poolKey, lpFee);
    }
    ...
    function beforeSwap(
        ...
        _sendPayment(poolKey, brunoIsToken0, paymentAmount);
    ...
    // implements the IBrunoMevModule interface
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return interfaceId == type(IBrunoMevModule).interfaceId;
    }
}
