...
import {IBrunoSniperAuctionV0} from "../BrunoSniperAuctionV0.sol";
...
// shared util for snipers to bid in BrunoSniperAuction V0 and V2 auctions
contract BrunoSniperUtilV2 is ReentrancyGuard {
    ...
    IBrunoSniperAuctionV0 immutable brunoSniperAuction;
    IUniversalRouter immutable universalRouter;
    IPermit2 immutable permit2;
    address immutable weth;

    constructor(
        address _brunoSniperAuction,
        address _universalRouter,
        address _permit2,
        address _weth
    ) {
        brunoSniperAuction = IBrunoSniperAuctionV0(_brunoSniperAuction);
        ...
        returns (uint256 txGasPrice)
    {
        ...
        if (desiredBidAmount % brunoSniperAuction.paymentPerGasUnit() != 0) {
            revert InvalidBidAmount();
        }

        // calculate the gas price for the bid amount
        txGasPrice = auctionGasPeg + (desiredBidAmount / brunoSniperAuction.paymentPerGasUnit());
    }
        ...
        nonReentrant
    {
        ...
        if (round != brunoSniperAuction.round(poolId) || round > brunoSniperAuction.maxRounds())
        {
            revert InvalidRound();
        }

        // check that the correct block is being bid in
        if (block.number != brunoSniperAuction.nextAuctionBlock(poolId)) {
            revert InvalidBlock();
        }

        // check that the gas price is high enough
        int256 paymentUnits = int256(tx.gasprice) - int256(brunoSniperAuction.gasPeg(poolId));
        if (paymentUnits < 0) {
            revert GasPriceTooLow();
        }

        // check proper msg.value was sent in
        uint256 expectedBidAmount = brunoSniperAuction.paymentPerGasUnit() * uint256(paymentUnits);
        ...
        IERC20(weth).approve(address(brunoSniperAuction), msg.value);
        ...
        if (
            IBrunoHookV2(address(swapParams.poolKey.hooks)).supportsInterface(
                type(IBrunoHookV2).interfaceId
            )
        ) {
            // v2 hook
            swapParams.hookData = abi.encode(
                IBrunoHookV2.PoolSwapData({
        ...
        if (round + 1 != brunoSniperAuction.round(poolId)) {
